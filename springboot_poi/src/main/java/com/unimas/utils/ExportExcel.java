...
public class ExportExcel {
     ...
     * 功能:真正实现导出
     */
    public static void doExportExcel(String sheetName,String titleName,List<String> headers,Collection<?> dataSet,String resultUrl,String pattern) {
    ...
    private static List<Object> doImportExcel(String originUrl,int startRow,int endRow,boolean showInfo,Class<?> clazz) throws IOException {
        ...
        try {
            ...
            for (int i = startRow; i <= lastRowNum + endRow; i++) {
                ...
                if (row != null) {
                    rowList.add(row);
                    System.out.println("第" + (i + 1) + "行："+showInfo+" "+false);
                    // 获取每一单元格的值
                    for (int j = 0; j < row.getLastCellNum(); j++) {
                        String value = getCellValue(row.getCell(j));
                        if (!value.equals("")) {
                            System.out.println(value + " | "+ showInfo+ ""+false);
                        }
                    }
